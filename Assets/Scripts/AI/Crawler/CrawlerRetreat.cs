...
public class CrawlerRetreat : State<Crawler>
{
    ...
    public override void UpdateAI(Crawler owner)
    {
        ...
        if (owner.destination != null && owner.nav2D.CalculatePath((Vector2)owner.destination, owner.nav2D.path) == false) //if the destination is unreachable assign a new one
        {
            owner.destination = null;
        }
