...
public class CrawlerIdle : State<Crawler>
{
    ...
    public override void UpdateAI(Crawler owner)
    {
        ...
        if (owner.destination == null || !owner.nav2D.CalculatePath((Vector2)owner.destination)) //just use nav2D to check that the position isn't unreachable
        {
            owner.destination = (Vector2)owner.transform.position + (Random.insideUnitCircle * Random.Range(5, 10));
            owner.cleared = false;
        }
